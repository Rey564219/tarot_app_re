...
SUIT_RANK_MEANINGS = {
    'Wands': {
        ...
        'Four': ('安定と祝福', ['達成', '休息', '喜び'], '落ち着かない', ['不満', '疲労', '不安定だが平和']),
        'Five': ('競争と摩擦', ['対立', '切磋', '挑戦'], '衝突回避', ['妥協', '回避', '疲れ','負ける']),
        'Six': ('勝利と評価', ['達成', '注目', '成功'], '過信と失速', ['傲慢', '失敗', '停滞','うしろめたさ']),
        'Seven': ('防衛と主張', ['踏ん張り', '防御', '努力'], '守り過ぎ', ['迷い', '不安', '弱気','キャパオーバー']),
        'Eight': ('急展開と加速', ['進行', '連絡', 'スピード'], '停滞と遅延', ['遅れ', '混乱', '障害','空回り']),
        'Nine': ('粘り強さ', ['耐久', '警戒', '継続'], '疲弊と限界', ['消耗', '警戒過多', '不安','迂闊さ']),
    ...
    'Cups': {
        'Ace': ('愛情の芽生え', ['感情', '優しさ', '直感'], '感情の停滞', ['失望', '閉塞', '不安','執着']),
        'Two': ('相思相愛', ['協力', '調和', '絆'], '不一致', ['誤解', '距離', '不安定','破局']),
        'Three': ('喜びの共有', ['祝福', '友情', '楽しさ'], '過度な享楽', ['散漫', '浪費', 'だらけ','二人だけの世界']),
        'Four': ('内省と退屈', ['熟考', '休息', '再評価'], '機会損失もしくは機械獲得', ['無気力', '後悔', '停滞']),
        'Five': ('喪失と悲しみ', ['失望', '後悔', '嘆き'], '受容と回復', ['立ち直り', '希望', '前進','残ったものに目を向ける']),
        'Six': ('思い出と純粋さ', ['回想', '優しさ', '郷愁'], '過去への執着', ['停滞', '依存', '後退']),
        'Seven': ('幻想と選択', ['夢', '迷い', '想像'], '現実への集中', ['決断', '整理', '目覚め']),
        'Eight': ('見切りと前進', ['離脱', '探索', '自立'], '踏み出せない', ['未練', '停滞', '迷い','戻ってくる']),
        'Nine': ('満足と充足', ['達成', '幸福', '余裕'], 'わがまま', ['過信', '不満', '虚無','恐怖の具現']),
        'Ten': ('家族の幸福', ['調和', '祝福', '完結'], '表面だけの幸せ', ['不和', '理想過多', '不満','時期が遅れる']),
        'Page': ('感受性と学び', ['直感', '優しさ', '好奇心','一途な恋'], '不安定', ['過敏', '気まぐれ', '迷い','浮気性']),
    ...
    'Swords': {
        ...
        'Two': ('静かな選択', ['保留', '均衡', '熟考'], '優柔不断もしくは選択', ['回避', '停滞', '不安','決断']),
        'Three': ('痛みと気づき', ['失恋', '悲しみ', '真実'], '回復の兆し', ['癒し', '受容', '前進','現状がわからないことによるヒステリック']),
        'Four': ('休息と回復', ['回復', '静養', '内省'], '不眠と不安','回復の兆し', ['疲労', '焦り', '消耗','目覚め']),
        'Five': ('対立と利益', ['衝突', '勝ち取る', '緊張'], '対立と敗北', ['妥協', '修復', '反省','敗北']),
        'Six': ('移動と癒し', ['移行', '回復', '旅','引っ越し'], '過去への執着', ['停滞', '後退', '迷い','原点回帰']),
        'Seven': ('策略と慎重', ['駆け引き', '工夫', '用心'], '露見と失敗、良きアドバイス', ['後悔', '失策', '警告']),
        'Eight': ('束縛と制限', ['閉塞', '不安', '孤立'], '解放の兆し', ['突破', '気づき', '前進']),
        'Nine': ('不安と恐れ', ['心配', '悪夢', '焦り','疑心暗鬼'], '緩和と回復', ['安心', '軽減', '立ち直り']),
        'Ten': ('終わりと決着', ['限界', '終了', '痛み','自業自得の死'], '再出発', ['回復', '再生', '希望']),
        'Page': ('好奇心と情報', ['学び', '観察', '警戒'], '噂と混乱', ['誤解', '軽率', '不信']),
        'Knight': ('迅速な行動', ['突破', '正義感', '決断'], '強引さ', ['衝動', '過激', '混乱']),
        'Queen': ('鋭さと自立', ['合理性', '洞察', '判断'], '冷たさ', ['辛辣', '孤立', '頑固','判断ミス']),
        'King': ('論理と統率', ['理性', '戦略', '公平'], '頑固と支配', ['独断', '冷徹', '偏り','残酷さ']),
    },
    'Pentacles': {
        'Ace': ('現実的な始まり', ['資金', '好機', '実行'], '機会損失', ['遅延', '不安', '不足']),
        'Two': ('バランスと調整', ['両立', '工夫', '柔軟'], '乱れ', ['不安定', '混乱', '過労']),
        'Three': ('協力と成果', ['評価', 'チーム', '成長'], '未熟', ['品質不足', '停滞', '不一致']),
        'Four': ('安定と保守','執着', ['確保', '節約', '安全','固執'], '執着と停滞からの解放の道', ['片付け', '不安', '手放す']),
        'Five': ('困難と不足', ['貧困', '不安', '孤立'], '回復の兆し', ['支援', '改善', '希望']),
        'Six': ('与える・受け取る', ['援助', '公平', '調和'], '偏り','支配と被支配', ['不公平', '負担', '不満']),
        'Seven': ('努力と待ち', ['成長', '評価待ち', '忍耐'], '焦り', ['停滞', '不満', '短気']),
        'Eight': ('熟練と積み上げ', ['練習', '技能', '努力'], '手抜き', ['中途半端', '雑', '不満']),
        'Nine': ('自立と実り', ['達成', '豊かさ', '余裕'], '孤独と過信', ['不安', '浪費', '虚無','詐欺まがい']),
        'Ten': ('長期的安定', ['家族', '資産', '継続'], '不安定もしくは過剰', ['崩れ', '過負担', '停滞']),
...
def _build_minor_meanings() -> dict:
    ...
    for suit, ranks in SUIT_RANK_MEANINGS.items():
        for rank, data in ranks.items():
            # Some entries have extra stray fragments; only use first 4 items.
            if not isinstance(data, (list, tuple)):
                continue
            if len(data) < 4:
                continue
            upright_meaning, upright_keywords, reversed_meaning, reversed_keywords = data[:4]
            name = f'{rank} of {suit}'
            meanings[name] = {
